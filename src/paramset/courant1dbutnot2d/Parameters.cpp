...
//GridConfig.param
      int GUARDSIZE                    = 1;
      int NUMBER_OF_CELLS_X            = 256; // these is for the INITIAL_OCTREE_REFINEMENT ...
      int NUMBER_OF_CELLS_Y            = 128;  // ... real number of cells will be higher
      int NUMBER_OF_CELLS_Z            = 3;    // ... at critical places
...
const double DELTA_T_SI                = 0.9*CELL_SIZE_SI / SPEED_OF_LIGHT_SI;
...
/* ABSORBER_SIDE: 0-left, 1-right, 2-bottom, 3-top, 4-back, 5-front */
Vec<bool,2*SIMDIM> ABSORBER_SIDE(0);
double ABSORBING_BORDER_THICKNESS      = 10*CELL_SIZE_X;
double ABSORBER_STRENGTH               = INF;

Vec<double,SIMDIM> SPAWN_AREA_SIZE     = Vec<double,SIMDIM>( CELL_SIZE_X, SIM_SIZE[1] );
Vec<double,SIMDIM> SPAWN_POS           = Vec<double,SIMDIM>( ABSORBING_BORDER_THICKNESS, 0.0 );
...
double ELLIPTIC_LENSE_SEMI_MINOR_AXIS  = 4.0 * LAMBDA;
